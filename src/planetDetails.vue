...
const checkIfFavorite = () => {
  ...
  if (planet.value && planet.value.name) {
    isFav.value = favorites.some(fav => fav.name === planet.value.name);
  }
};

const toggleFavorite = () => {
  let favorites = JSON.parse(localStorage.getItem('favorites')) || [];
  const currentPlanet = planet.value;
  
  if (!currentPlanet) return;

  const index = favorites.findIndex(fav => fav.name === currentPlanet.name);

  if (index > -1) {
    favorites.splice(index, 1);
    isFav.value = false;
  } else {
    favorites.push(currentPlanet);
    isFav.value = true;
  }
      ...
      <div v-if="planet" class="details-content">
        ...
        <div class="row align-items-center mt-4">
          ...
          <div class="col-md-7">
            ...
            <button @click="toggleFavorite" class="favorite-toggle-btn" :class="{ 'active': isFav }">
